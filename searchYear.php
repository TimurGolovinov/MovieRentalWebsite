...
?>
  <div class="row">
        <div class="container first-block top-10-headline">
            <h1 style="margin-bottom:1em; text-align:center; font-size: 4em;">Movie Searched by Year </h1>
        </div>
    </div>
    <div class="row">
        <div class="container">
            ...
            try 
            {
                ...
                $stmt1 = $conn->prepare('SELECT Title, Genre, Year FROM mytable WHERE Year = :Year');
                //run sql query for update search count for movie found
                $stmt2 = $conn->prepare('UPDATE mytable SET searchCounter = searchCounter + 1 WHERE Year = :Year');
